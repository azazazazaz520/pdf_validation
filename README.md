...
服务文件为 `pdf_to_word_service.py`，默认使用常驻的 `structure-table-lite`，任务状态保存在内存，任务文件保存在 `service_data\jobs`，过期任务按 TTL 清理。
...
默认仅监听 `127.0.0.1:8765`。部署到服务器时，应通过防火墙或反向代理限制访问范围；确需局域网访问时再设置 `PDF_SERVICE_HOST=0.0.0.0`，并保留 `PDF_SERVICE_TOKEN`。
...
验证目录中的 `fixtures` PDF 是合成样本；真实扫描 PDF、复杂表格、多栏文档、旋转页面、图片和公式仍需补充样本后单独验收。DOCX 结构检查已完成；当前环境缺少 LibreOffice，因此尚未完成 DOCX 的 PNG 渲染级视觉检查。
